...
//A LinSearch function and a BinSearch function
int LinSearch(int array[], int x);
int BinSearch(int array[], int x);

int main(void)
{
  int x; //The number we are searching for
  char filename[20]; //Search through 5 different text documents
  FILE *myFile;
  int array[N]; //Sætter N ind i en array
  int k; //
  ...
  if (result == 0) printf("%d is not in the list \n",x);
  else printf("%d is in position %d\n", array[result], result+1);
